...
const AgentChat: React.FC<AgentChatProps> = ({ agent }) => {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(err);
      const errorMessage =
        'Lo siento, ha ocurrido un error al contactar al modelo de IA.';
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <form
        ...
        className="flex items-center space-x-2 border-t pt-4 flex-shrink-0"
      >
        <input
          type="text"
          value={input}
          ...
          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#232A37] text-gray-600"
          disabled={isLoading}
        />
        <button
          type="submit"
